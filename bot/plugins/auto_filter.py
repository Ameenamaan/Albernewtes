...
from config import start_uptime 
from plugins.running import FIND, INVITE_LINK, ACTIVE_CHATS, recacher, gen_invite_links
...
async def cb_settings(bot, update: CallbackQuery):
    ...
    text =f"<b><u>{bot_fname}'s</u></b> Settings Pannel.....\n"
...
async def cb_config(bot, update: CallbackQuery):
    ...
    buttons.append([
       InlineKeyboardButton("✌️Total Filter Count✌️", callback_data=f"mf_count({mf_count}|{chat_id})")
       ]
    )

    buttons.append([
       InlineKeyboardButton("⤵️Accuracy⤵️", callback_data=f"accuracy({accuracy_point}|{chat_id})")
       ]
    )
